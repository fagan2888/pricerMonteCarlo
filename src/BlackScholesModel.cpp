...
void BlackScholesModel::asset(PnlMat *path, double T, int nbTimeSteps, PnlRng *rng)
{
    ...
    for (int i = 1; i <= nbTimeSteps;i++) {
        ...
        for (int d=0; d < size_ ; d++) {
            long double sigma_d=pnl_vect_get(sigma_,d);
            PnlVect* L_d=pnl_vect_create(size_);
            pnl_mat_get_row(L_d,gamma,d);
            long double path_t_d=pnl_mat_get(path,i-1,d)*exp((r_-pow(sigma_d,2)/2)*(T/nbTimeSteps)+sigma_d*sqrt(T/nbTimeSteps)*pnl_vect_scalar_prod(L_d,G_i));
