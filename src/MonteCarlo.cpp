...
void MonteCarlo::price(double &prix, double &ic) {
    PnlMat *spotsMat = pnl_mat_create(opt_->nbTimeSteps_ + 1, mod_->size_);
...
void MonteCarlo::price(const PnlMat *past, double t, double &prix, double &ic) {
    ...
    PnlMat *pathMat = pnl_mat_create(opt_->nbTimeSteps_ + 1, mod_->size_);
    double tempPayoff = 0.0;
    double squareSum = 0.0;
    double sum = 0.0;
    for (int i = 0; i < nbSamples_; i++) {
        mod_->asset(pathMat, t, opt_->T_, opt_->nbTimeSteps_, rng_, past);
